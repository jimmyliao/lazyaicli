...
# --- symlink engine onto PATH ----------------------------------------------
mkdir -p "$BIN_DIR"
ln -sf "$REPO_DIR/bin/ccs" "$BIN_DIR/ccs"
echo "✓ linked $BIN_DIR/ccs -> $REPO_DIR/bin/ccs"
case ":$PATH:" in
  *":$BIN_DIR:"*) ;;
...
if [ -n "$RC" ]; then
  if grep -qF "$REPO_DIR/ccs.sh" "$RC" 2>/dev/null; then
    ...
    printf '\n# agent-cli-sessions (ccs) — stay in the resumed session dir after exit\nsource "%s/ccs.sh"\n' "$REPO_DIR" >> "$RC"
    echo "✓ added source line to $RC"
  fi
  echo ""
  echo "Done. Restart your shell or:  source $RC"
else
  echo "⚠ Unknown shell ($SHELL). Add this line to your shell rc manually:"
